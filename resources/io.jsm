...
var EXPORTED_SYMBOLS = ["OFFEN.FileIO", "OFFEN.DirIO"];
...
OFFEN.FileIO = {
	...
	open   : function(path) {
		try {
		  ...
		  OFFEN.Util.log(OFFEN.Util.logC(),"open - EXCEPTION");
	...
	write  : function(file, data, mode, charset) {
      ...
      OFFEN.Util.log(OFFEN.Util.logC(),"write - EXCEPTION");
	...
	unlink : function(file) {
  	  ...
  	  OFFEN.Util.log(OFFEN.Util.logC(),"unlink - EXCEPTION");
			return false;
		}
	},
  unlinkfile : function(filename) {
		try {
      var file = OFFEN.FileIO.open(filename);
      file.remove(false);
			return true;
		}
		catch(e) {
  	  OFFEN.Util.log(OFFEN.Util.logC(),"unlinkfile - EXCEPTION");
			return false;
		}
	},
	testfilesize : function(filename,size) {
		try {
      var file = OFFEN.FileIO.open(filename);
   	  OFFEN.Util.log(OFFEN.Util.logA(),"testfilesize - opening "+filename);

      OFFEN.FileIO.write(file," ");
      file.fileSize = size;
      file.remove(false);
			return true;
		}
		catch(e) {
      OFFEN.Util.log(OFFEN.Util.logC(),"testfilesize - EXCEPTION");
  	  return false;
		}
	},
	path   : function(file) {
		...
		try {
			while (dirEntry.hasMoreElements()) {
				list.push(dirEntry.getNext()
								.QueryInterface(OFFEN.FileIO.localfileIID));
			}
			if (recursive) {
				var list2 = new Array();
